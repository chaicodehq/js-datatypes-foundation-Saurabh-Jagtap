...
export function isValidPincode(code) {
  ...
  if (code.startsWith('0', 0)) return false

  if (code.length !== 6) return false

  let regex = /^\d+$/
  for (const i of code) {
    if(!regex.test(i)) return false
  }
...
export function formatPostcardField(label, value, width = 12) {
  ...
  if(width !== 0 || width !== undefined) return label.padEnd(width) + ": " + value;
...
export function countVowels(message) {
  ...
  let vowelCount = message.match(vowelRegex);
  if(vowelCount === null || vowelCount.length === 0 ) return 0;
